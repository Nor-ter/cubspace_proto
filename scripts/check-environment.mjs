...
function check(label, path, args = ['--version']) {
  if (!contained(path)) {
    console.error(`FAIL ${label}: 환경 내부 설치를 찾지 못했습니다 (${path})`);
    failures++;
    return;
  }
  ...
  if (result.error || result.status !== 0) {
    console.error(
      `FAIL ${label}: ${result.error?.message || result.stderr.trim()}`,
    );
    failures++;
  } else
    console.log(
...
if (!contained(codex)) {
  console.error('FAIL Codex: 환경 내부 Codex가 없습니다.');
  failures++;
} else check('Codex', process.execPath, [codex, '--version']);
check(
  'Claude Code',
  windows
