# CUB-100: Taskcard 100

## 작업 내용

최근 온보딩 웹사이트와 자동화 변경을 Task / Perform task card 아래 Taskcard 100으로 게시한다. 작업 문서와 실행 ID는 CUB-100처럼 날짜 없이 사용한다. 검사와 독립 리뷰를 통과한 결과, 실행 시간, Git 링크를 ClickUp에 전달하고 review 상태로 설정한다. workflow push 명령에서 Git push 성공 후 ClickUp 게시까지 수행한다.
...
- publish: 부모와 상태를 검증하고 중복 subtask를 생성하지 않는다.
- retry: 서버 응답이 불확실하면 생성 요청을 자동 반복하지 않는다.
- short id: 같은 ID 재생성 시 기존 이력을 보존한다.
